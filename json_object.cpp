...
namespace rapidjson_ext
{
    ...
    JsonObject& JsonObject::Add()
    {
        ...
        JsonObject* v = new JsonObject(allocator_);
        v->parent_ = this;
        array_.push_back(v);
        return *v;
    }
    ...
    void JsonObject::BuildJsonObjectTree()
    {
        if ( NULL == value_)
        {
            return;
        ...
        if (HasMember())
        {
            ...
            {
                it->second->BuildJsonObjectTree();
                it++;
            }
            for (int i = 0; i < (int) array_.size(); i++)
            {
                array_[i]->BuildJsonObjectTree();
            }
        }
        if (parent_ != NULL)
        {
            if (parent_->value_->IsObject())
            {
                //TLOG_ERROR("add:" << _name << " with " << getString() << " && "<< (int)(_value.GetType()) << endl);
                rapidjson::Value::StringRefType nv(name_.data(), name_.size());
                parent_->value_->AddMember(nv, *value_, *allocator_);
            }
            else
            {
                parent_->value_->PushBack(*value_, *allocator_);
            }
        }
    }
